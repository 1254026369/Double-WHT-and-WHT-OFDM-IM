function [Rx_bin_index, Rx_bin_info,index_symbol_de,infor_symbol_de] = OFDM_IM_RX(OFDM_IM_Para, Rx_Sample,N_sigma,bit,EbN0dB)
...
%     N_sigma = 1; 测试
 ...
 elseif detection == 3         %MP判决
   ...
   elseif detection == 4         %MP判决
    for k = 1: N_frm
        [index_symbol_de(:,:,k),infor_symbol_de(:,:,k)] = N_MP_detection(K_sc_used,Rx_Sym_SC_Eq(:, :, k),2^Bit_Loading_SC(1),N_Symbol,abs(mean(conj(CRdata)./abs(CRdata).^2))^2*N_sigma,EbN0dB);
    end
 end
