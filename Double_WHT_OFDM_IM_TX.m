function [output,output2,output3,index_bit,info_bit] = Double_WHT_OFDM_IM_TX(OFDM_IM_Para,bit)
%% OFDM_TX  OFDM_IM传输机
...
Hada = hadamard(NN);
% if NN~= N_sc_used
%     Tx_Sym_FFT(:,N_sc_used + 1:NN) = 0;
% end
Tx_Sym_FFT_WHT =  Tx_Sym_FFT * Hada; 
% scatterplot(Tx_Sym_FFT_WHT(:));
...
%     scatterplot(Double_OFDM_Window_WHT(:));
    N_CP = ceil(CP * N_fft);                        % sample length of cyclic prefix 循环前缀采样长度
...
Double_OFDM_I_Signal_WHT = real(Double_OFDM_Signal_WHT);  
Double_OFDM_I_Signal_WHT = Double_OFDM_I_Signal_WHT - mean(Double_OFDM_I_Signal_WHT);         
Double_I_V_max_WHT = max(abs(Double_OFDM_I_Signal_WHT));
Double_OFDM_Q_Signal_WHT = imag(Double_OFDM_Signal_WHT);  
Double_OFDM_Q_Signal_WHT = Double_OFDM_Q_Signal_WHT - mean(Double_OFDM_Q_Signal_WHT); %remove the DC component 除去直流分量
Double_Q_V_max_WHT = max(abs(Double_OFDM_Q_Signal_WHT));
% Sig_Upsampled_r = real(Sig_Upsampled);
% Sig_Upsampled_i = imag(Sig_Upsampled);
Double_dv_r_WHT = Double_I_V_max_WHT*2/(2^N_bits);
Double_dv_i_WHT = Double_Q_V_max_WHT*2/(2^N_bits);
Double_SigOut_DAC_r_WHT = (floor(Double_OFDM_I_Signal_WHT./ Double_dv_r_WHT) + 1/2) .* Double_dv_r_WHT;
Double_SigOut_DAC_i_WHT = (floor(Double_OFDM_Q_Signal_WHT ./ Double_dv_i_WHT ) + 1/2) .* Double_dv_i_WHT ;
Double_SigOut_DAC_WHT = Double_SigOut_DAC_r_WHT+Double_SigOut_DAC_i_WHT*i;
% d_V = 2 * V_max / (2^N_bits);
% SigOut_DAC = (floor(Sig_Upsampled ./ d_V) + 1/2) .* d_V;
output3 = Double_SigOut_DAC_WHT;
